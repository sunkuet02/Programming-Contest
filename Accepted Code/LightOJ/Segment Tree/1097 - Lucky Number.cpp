...
void update(int no , int b, int e, int i, int j,int cnt)
{
    ...
    if( (tree[no*2].cnt < i ))
    {
        update(no*2 +1 , mid +1 , e , i -tree[no*2].cnt ,j - tree[no*2].cnt, 0) ;
    }
    else
    {
        update(no*2, b , mid , i , j,0);
    }
...
int main()
{
    ...
    buildTree(1,1,sz-2);
//
//    update(1,1,sz-2, 3,3,0);
//    update(1,1,sz-2, 3,3,0);
    //cout << queryTree(1,1,sz-1,3,3) << endl;;
    for(int i = 2; i<=100000; i++ )
    {
        int cur = queryTree(1,1,sz-1,i,i);
        //cout << cur<< endl;
        int k = 0 ;
        update(1,1,sz-2, cur,cur,0);
        for(int j = cur + cur -1; j<= 1000004 ; j+=(cur-1) )
        {
            update(1,1,sz-2, j,j,0);
            k++;
        }
    }
    f = true;
