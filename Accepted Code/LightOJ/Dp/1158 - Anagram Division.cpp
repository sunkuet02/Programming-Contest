...
int cal(int mask, int modVal)
{
    ...
    for(int i = 0 ; i< n; i++ )
    {
        if(!checkbit(mask,i))
        {
            ret += cal(setbit(mask,i), (modVal*10 + (str[i] - 48) )%d ) ;
        }
    }
    return ret;
}
...
int main()
{
    ...
    rep(t,1,test)
    {
        ...
        int ret = cal(0,0);
        for(int i = 0 ; i< n; i++ ) digitCount[ str[i] - 48 ] ++ ;
        for(int i = 0; i<=9 ; i++)  ret /= fact[digitCount[i]];
