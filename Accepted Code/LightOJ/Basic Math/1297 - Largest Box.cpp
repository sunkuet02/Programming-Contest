...
/*
    The equation : (L-2*x)*(W-2*x)*x = 4*x^3 - 2*x^2*(W+L) + W*L*x
    First Derivativ : 12*x^2 - 4*x*(W+L) + w*L
    The value of x :( 4*(W+L)(+|-) sqrt(16*(W+L)^2 - 48*W*L) )/2*12
    Second Derivating: 24*x - 4*(W+L)

    From these equations we will get the  x for which the equations
    will gives maximum and minimum value.

*/

double w, l;

double retXvalue(bool forPlusSign)
{
    double sqVal = sqrt(16*(w+l)*(w+l) - 48*w*l) ;
    if(forPlusSign)
    {
        return (4*(w+l) + sqVal)/24.0 ;
    }
    else
    {
        return (4*(w+l) - sqVal)/24.0 ;
...
bool check2ndDerivativeVal(double x)
{
    double val = 24*x - 4*(w+l);
...
double getTheXValue()
{
    double xVal = retXvalue(true) ;
    if ( check2ndDerivativeVal(xVal) )
    {
        return xVal;
    }

    return retXvalue(false) ;
}

double equationValue(double x)
{
    return 4*x*x*x - 2*x*x *(w+l) + w*l*x;
}
int main()
{
    //Read;
    ...
    {
        scanf("%lf %lf",&w,&l);
        double x = getTheXValue();
        double ans = equationValue(x) ;
