...
int cal(int pos, int upToVal) {
	if(pos == n){
		if(upToVal > 2000 || upToVal <= m ) return 0;
		return -1111111;
	}
	
	int & ret = dp[pos][upToVal];
	if(ret!=-1) return ret;
	
	if(upToVal + v[pos].second <= m + 200)
		ret = max(cal(pos+1, upToVal+v[pos].second) + v[pos].first, cal(pos+1, upToVal));
	else 
		ret = cal(pos+1, upToVal);
	return ret;
}
