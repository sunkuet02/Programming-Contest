...
int main() {
    ...
    while(scanf("%d %d",&n, &m)!=EOF) {
        ...
        while(m--) {
            ...
            int lower = lower_bound(numbers.begin(), numbers.end(), pii(v,0), compare ) - numbers.begin();
            
            if(lower + (k-1) < numbers.size()) {
                if(numbers[lower + (k-1)].first == v) {
                    printf("%d\n",numbers[lower + (k-1)].second);
                } else {
                    printf("0\n");
                }
            } else {
