...
int D;
int binarySearch(int d, int cur, int i)
{
    if(i ==1 && d == D) return cur;
    if(i&1)
    {
        return binarySearch(d + 1, cur*2, i/2+1);
    }
    else
    {
        return binarySearch(d+1 , cur* 2 +1, i/2);
    }
}
